...
function LocalStress(VxN, VxS, VyN, VyS, P, P0, τxx0, τyy0, τzz0, τxy0, G, K, ϕ, ψ, c, ηvp, Δy, Δt)
    ...
    return τxx, τyy, τzz, τxy, Pc, fc
end

function ComputeStress!(τxx, τyy, τzz, τxy, Pc, Fc,  x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
    ...
    for i=1:length(NumV.x)+1
        ...
        τ11, τ22, τ33, τ21, pc, fc = LocalStress( VxN, VxS, VyN, VyS, p, P0[i], τxx0[i], τyy0[i], τzz0[i], τxy0[i], rheo.G[i], rheo.K[i], rheo.ϕ[i], rheo.ψ[i], rheo.c[i], rheo.ηvp[i], Δy, Δt )
        τxx[i]   = τ11
        τyy[i]   = τ22
        τzz[i]   = τ33
        τxy[i]   = τ21
...
function main(σ0)
    ...
    for it=1:Nt
        ...
        for iter=1:100
            ...
            @show (iter, norm(F))
            if norm(F)/length(F)<1e-10 break end
        ...
        if norm(F)/length(F)>1e-10 error("Diverged!") end
        ...
        ComputeStress!(τxx, τyy, τzz, τxy, Pc, fc, x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
        P .= Pc #!!!!!!!!!
