...
# I've tried using AD to solve BC values. This works. 
# However it does not work when called within the the residual function.
# Because the residual function is also suject to AD for Jacobian assembly  
...
function ComputeStress!(τxx, τyy, τzz, τxy, Pc, Fc, Pl, εyyt,  x, P, P0, τxx0, τyy0, τzz0, τxy0, BC, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
    ...
    for i=1:length(NumV.x)+1
        ...
        elseif i==length(NumV.x)+1 # Special case for top (North) vertex
            ...
            # elseif BC.Vx[end]==2
            #     VxN = (τxyBC-τxy0[end])/(rheo.G[end]*Δt)*Δy + x[iVx-1]
            # end
            # if BC.Vy[end]==1
    ...
    for i=1:length(NumV.x)
        ...
        if i==iV.x
            ...
            # if BC.Vx[end]==1 
                VxN = 2 *V_BC.x.N - VxC
            # elseif BC.Vx[end]==2 
            #     VxN = (τxyBC-τxy0[end])/(rheo.G[end]*Δt)*Δy + VxC
            # end
            # if BC.Vy[end]==1 
            #     VyN = 2*V_BC.y.N - VyC 
            # elseif BC.Vy[end]==2 
                VyN = NeumannVerticalVelocity(σ_BC.yy.N, VyC, rheo.G[end], rheo.K[end], τyy0[end], P0[end], Δy, Δt)
                # VyN = NeumannVerticalVelocityAD(σ_BC.yy.N, VyC, rheo.G[end], rheo.K[end], τyy0[end], P0[end], Δy, Δt)
            # end
        else
            VxN = x[iVx+1]
            VyN = x[iVy+1]
...
function main(σ0)
    ...
    for it=1:Nt
        ...
        if mod(it, nout)==0
            ...
            p4 = plot(εyyt[1:end-1]*100, yv[1:end-1])
