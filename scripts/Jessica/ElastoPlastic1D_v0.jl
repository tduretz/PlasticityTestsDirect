...
function LocalStress(VxN, VxS, VyN, VyS, P, P0, τxx0, τyy0, τzz0, τxy0, G, K, ϕ, ψ, c, ηvp, Δy, Δt)
    ...
    return τxx, τyy, τzz, τxy, Pc, fc, pl
end

function ComputeStress!(τxx, τyy, τzz, τxy, Pc, Fc, Pl,  x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
    ...
    for i=1:length(NumV.x)+1
        ...
        τ11, τ22, τ33, τ21, pc, fc, pl = LocalStress( VxN, VxS, VyN, VyS, p, P0[i], τxx0[i], τyy0[i], τzz0[i], τxy0[i], rheo.G[i], rheo.K[i], rheo.ϕ[i], rheo.ψ[i], rheo.c[i], rheo.ηvp[i], Δy, Δt )
        τxx[i]   = τ11
        τyy[i]   = τ22
        τzz[i]   = τ33
        τxy[i]   = τ21
...
function Res!(F, x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
    ...
    for i=1:length(NumV.x)
        ...
        τxxN, τyyN, τzzN, τxyN, PN = LocalStress( VxN, VxC, VyN, VyC, PN, P0[i+1], τxx0[i+1], τyy0[i+1], τzz0[i+1], τxy0[+1i], rheo.G[i+1], rheo.K[i+1], rheo.ϕ[i+1], rheo.ψ[i+1], rheo.c[i+1], rheo.ηvp[i+1], Δy, Δt )
...
function main(σ0)
    ...
    rheo.c[Int64(floor(Ncy/2))] = params.c/sc.σ
    # rheo.c[Int64(floor(Ncy/2))] = 10/100/sc.σ
    ...
    Res_closed! = (F,x) -> Res!(F, x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
    ...
    for it=1:Nt
        ...
        for iter=1:100
            ...
            # Residual
            Res!(F, x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
            @show (iter, norm(F))
        ...
        ComputeStress!(τxx, τyy, τzz, τxy, Pc, fc, pl, x, P, P0, τxx0, τyy0, τzz0, τxy0, V_BC, σ_BC, rheo, iV, NumV, Δy, Δt )
        P .= Pc #!!!!!!!!!
        ...
        if mod(it, nout)==0
            p3 = plot()
            p3 = plot!(Vx, yc, label="Vx")
            p3 = plot!(Vy, yc, label="Vy")
            p3 = plot!(P,  yv, label="P")
            p3 = scatter!(P[pl.==1], yv[pl.==1])
